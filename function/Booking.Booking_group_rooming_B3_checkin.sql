...
BEGIN
  ...
  -- No option
  IF NEW."Check_in_option_id" IS NULL OR NEW."Room_id" IS NULL THEN
  ...
  WHERE "Payment_type" = 'checkin'
    AND "Booking_group_id" = NEW."Booking_id";
  IF num > 0 THEN
    RETURN NEW;
  END IF;
  ...
  IF price > 0 THEN
    ...
    INSERT
      ...
      VALUES (payment_method_id, 'cotown', payer, NEW."Booking_id", price, CURRENT_DATE, 'Check-in ' || resource, 'checkin');
  END IF;
