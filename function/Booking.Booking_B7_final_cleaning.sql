...
BEGIN
  ...
  IF NEW."Date_from" >= '2024-06-01' AND NEW."Final_cleaning" > 0 THEN
    SELECT id, "Name", "Tax_id" into product_id, product_name, tax_id FROM "Billing"."Product" WHERE "Name" LIKE 'Limpieza de salida%' LIMIT 1;
    IF product_id IS NOT NULL THEN
      ...
      IF NOT EXISTS (SELECT id FROM "Booking"."Booking_service" WHERE "Booking_id" = NEW.id AND "Product_id" = product_id) THEN
        ...
        VALUES (NEW.id, 'puntual', NEW."Date_from", product_id, 1, product_name, NEW."Final_cleaning", tax_id, payment_method_id);
      END IF;
