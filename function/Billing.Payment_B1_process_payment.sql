...
BEGIN
  ...
  -- Sub transaccion: un fallo del workflow de la reserva no puede tumbar el registro del cobro
  BEGIN
    ...
    IF (NEW."Payment_type" = 'deposito' AND NEW."Booking_id" IS NOT NULL) THEN
      ...
      IF status_record IN ('confirmada', 'documentacionok', 'firmacontrato')
         AND (status_record = 'documentacionok' OR booking_fee = 0 OR booking_fee_actual IS NOT NULL) THEN
  ...
  -- El pago se registra igualmente, solo se anota el fallo del workflow
  EXCEPTION WHEN OTHERS THEN

    GET STACKED DIAGNOSTICS err = MESSAGE_TEXT, err_state = RETURNED_SQLSTATE;

    -- Anota el error en la reserva (sin poder tumbar el registro del pago)
    IF NEW."Booking_id" IS NOT NULL THEN
      BEGIN
        INSERT INTO "Booking"."Booking_log" ("Booking_id", "Log")
        VALUES (NEW."Booking_id", CONCAT('ERROR: pago ', NEW.id, ' (', NEW."Payment_type", ', ', NEW."Amount", ') cobrado y registrado, pero NO se ha podido actualizar la reserva: ', err_state, ' ', err));
