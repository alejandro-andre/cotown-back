...
def load_stabilised(dbClient, con, data):
  ...
  for irow, row in enumerate(data.iter_rows(min_row=3)):
    ...
    try:
      ...
      sql = 'INSERT INTO "Resource"."Resource_stabilised" ({}) VALUES ({}) ON CONFLICT ("Resource_id", "Date_price") DO UPDATE SET {} RETURNING ID'.format(','.join(fields), ','.join(markers), ','.join(update))
