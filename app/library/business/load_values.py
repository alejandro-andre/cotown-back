...
def load_values(dbClient, con, data):
  ...
  try:
    sql = '''
    DELETE FROM "Resource"."Resource_value" 
    ...
    logger.info('Valuations deleted')
  except:
    logger.error(error)
    con.rollback()
    return False, error
  ...
  for irow, row in enumerate(data.iter_rows(min_row=3)):
    ...
    try:
      ...
      sql = 'INSERT INTO "Resource"."Resource_value" ({}) VALUES ({}) ON CONFLICT ("Resource_id", "Valuation_date") DO UPDATE SET {} RETURNING ID'.format(','.join(fields), ','.join(markers), ','.join(update))
