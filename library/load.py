...
def load_resources(dbClient, resources):
    ...
    # Header
    header = list(map(lambda cell: cell.value, resources[2]))

    # Loop thru all rows skipping two first rows
    for irow, row in enumerate(resources.iter_rows(min_row=3)):
        ...
        for icol, cell in enumerate(row):
            ...
            elif column == 'Resource_flat_type.Code':
                id = None
                if cell.value != '':
            ...
            elif column == 'Resource_place_type.Code':
                id = None
                if cell.value != '':
